	...
	for (int i = 0; i<Keyboard::MAX_ROW + 3; i++)
	{
		...
		for (int row = 0; row<Keyboard::MAX_ROW; row++)
		{
			for (int col = 0; col<Keyboard::MAX_COLUMN; col++)
			{
				...
				{
					Effect.Color[row][col] = RGB(GetRValue(myColorRef),
						GetGValue(myColorRef),
						GetBValue(myColorRef));
				}

				if ((i - 1) == row)
				{
					Effect.Color[row][col] = RGB(0.3 * GetRValue(myColorRef),
						0.3 * GetGValue(myColorRef),
						0.3 * GetBValue(myColorRef));
				}

				if ((i - 2) == row)
				{
					Effect.Color[row][col] = RGB(0.1 * GetRValue(myColorRef),
						0.1 * GetGValue(myColorRef),
						0.1 * GetBValue(myColorRef));
				}
			}
		}
        ...
        if(CreateKeyboardEffect)
        {
            ...
            else
            {
                ...
                for(UINT i=0; i<ActiveKeysData.numKeys; i++)
                {
                    ...
                    switch(ActiveKeysData.Key[i])
                    {
					case 0x30: Effect.Key[HIBYTE(RZKEY_0)][LOBYTE(RZKEY_0)] = Color; break;
					case 0x31: Effect.Key[HIBYTE(RZKEY_1)][LOBYTE(RZKEY_1)] = Color; break;
					case 0x32: Effect.Key[HIBYTE(RZKEY_2)][LOBYTE(RZKEY_2)] = Color; break;
					case 0x33: Effect.Key[HIBYTE(RZKEY_3)][LOBYTE(RZKEY_3)] = Color; break;
					case 0x34: Effect.Key[HIBYTE(RZKEY_4)][LOBYTE(RZKEY_4)] = Color; break;
					case 0x35: Effect.Key[HIBYTE(RZKEY_5)][LOBYTE(RZKEY_5)] = Color; break;
					case 0x36: Effect.Key[HIBYTE(RZKEY_6)][LOBYTE(RZKEY_6)] = Color; break;
					case 0x37: Effect.Key[HIBYTE(RZKEY_7)][LOBYTE(RZKEY_7)] = Color; break;
					case 0x38: Effect.Key[HIBYTE(RZKEY_8)][LOBYTE(RZKEY_8)] = Color; break;
					case 0x39: Effect.Key[HIBYTE(RZKEY_9)][LOBYTE(RZKEY_9)] = Color; break;
                    case 0x41: Effect.Key[HIBYTE(RZKEY_A)][LOBYTE(RZKEY_A)] = Color; break;
                    case 0x42: Effect.Key[HIBYTE(RZKEY_B)][LOBYTE(RZKEY_B)] = Color; break;
                    case 0x43: Effect.Key[HIBYTE(RZKEY_C)][LOBYTE(RZKEY_C)] = Color; break;
                    case 0x44: Effect.Key[HIBYTE(RZKEY_D)][LOBYTE(RZKEY_D)] = Color; break;
...
void CChromaSDKImpl::ShowDamageEffect(UINT DeviceType)
{
    ...
    case 1:
        if(CreateKeyboardEffect)
        {
            ...
            // Flash red quickly
            CreateKeyboardEffect(ChromaSDK::Keyboard::CHROMA_REACTIVE, &HitEffect, NULL);
            Sleep(50);
